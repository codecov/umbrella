...
def test_ta_timeseries_timeout_defaults():
    assert db_settings.TA_TIMESERIES_CONNECT_TIMEOUT == 5
    assert db_settings.TA_TIMESERIES_STATEMENT_TIMEOUT_MS == 10_000


def test_ta_timeseries_database_includes_options_when_enabled():
    ...
    assert (
        ta_db["OPTIONS"]["options"]
        == f"-c statement_timeout={db_settings.TA_TIMESERIES_STATEMENT_TIMEOUT_MS}"
    )
