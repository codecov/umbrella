        ...
        for current_retry in range(1, max_retries + 1):
            ...
            try:
                ...
                if current_retry >= max_retries:
                    raise TorngitServerUnreachableError(
                        "GitLab was not able to be reached due to a timeout or network error. Please try again."
                    )
                log.warning(
                    "GitLab request timed out or network error, retrying",
                    extra=dict(retry=current_retry, **_log),
                )
                continue
