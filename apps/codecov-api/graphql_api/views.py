...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        # native GraphQL errors (validation, type coercion) have no original_error;
        # they are client-side mistakes (e.g. wrong variable type) not server errors
        is_native_graphql_error = error.original_error is None
        ...
        if is_native_graphql_error:
            # client sent invalid variables/query — not a server error, don't capture
