...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        if isinstance(original_error, BaseException) or isinstance(
            ...
            formatted["type"] = type(original_error).__name__
        elif original_error is None or isinstance(original_error, GraphQLSyntaxError):
            # Core GraphQL parse/validation errors (e.g. syntax errors, unknown fields)
            # are caused by malformed client queries — not server faults.
            # Do not capture to Sentry; the masked response is sufficient.
            pass
        else:
