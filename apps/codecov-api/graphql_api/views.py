...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        # native GraphQL variable/schema validation errors have no original_error
        is_variable_error = error.original_error is None
        if debug or (not is_anonymous and (is_bad_query or is_variable_error)):
            return format_error(error, debug)
        formatted = error.formatted
        formatted["message"] = "INTERNAL SERVER ERROR"
        formatted["type"] = "ServerError"
        # if this is a native GraphQL validation error (e.g. wrong variable type),
        # it's a client mistake — don't log or send to Sentry
