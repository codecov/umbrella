...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        # Pure GraphQL validation errors (e.g. unknown fields, missing subfield
        # selections) never have an original_error — they are client mistakes, not
        # server faults.  Return the descriptive GraphQL message directly without
        # logging or sending to Sentry.
        is_graphql_validation_error = error.original_error is None
        # Legacy string-match kept for backwards compatibility
        is_bad_query = "Cannot query field" in error.formatted["message"]
        if debug or is_graphql_validation_error or (not is_anonymous and is_bad_query):
