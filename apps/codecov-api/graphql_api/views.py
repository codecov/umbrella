...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        # GraphQL validation/syntax errors have no original_error — they are raised
        # by the GraphQL engine during query parsing/validation (e.g. missing
        # subfield selections, unknown fields) and are always client mistakes.
        # Return them directly without masking or capturing to Sentry.
        if debug or original_error is None:
            return format_error(error, debug)

        # Legacy check: surface the raw error message to authenticated users for
        # bad-field queries so they can self-diagnose.
        is_bad_query = "Cannot query field" in error.formatted["message"]
        if not is_anonymous and is_bad_query:
            return format_error(error, debug)
