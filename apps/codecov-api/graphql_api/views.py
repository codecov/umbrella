...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        if isinstance(original_error, BaseException) or isinstance(
            ...
            # GraphQL parsing/syntax/validation errors (e.g. GraphQLSyntaxError)
            # have no original_error — these are client errors and should not be
            # captured in Sentry.
            formatted["type"] = type(error).__name__
            formatted["message"] = (
                error.message if not is_anonymous else "Invalid query"
            )
        else:
