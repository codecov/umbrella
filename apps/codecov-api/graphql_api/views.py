...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        # GraphQL schema validation errors (e.g. missing required argument, unknown
        # field, invalid variable type) have no underlying original_error — they are
        # client-side mistakes, not internal server failures.  Return a generic
        # validation error response without logging to Sentry.
