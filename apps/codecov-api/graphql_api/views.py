...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        # GraphQL validation errors (syntax/schema errors caught before execution)
        # have no original_error; treat them as client errors, not server faults.
        is_validation_error = error.original_error is None
        # the only way to check for a malformed query
        is_bad_query = "Cannot query field" in error.formatted["message"]
        if debug or (not is_anonymous and is_bad_query) or is_validation_error:
