...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        if isinstance(original_error, BaseException) or isinstance(
            ...
            # Schema validation errors from malformed client queries (e.g. querying
            # a non-existent field) are client errors, not server errors — skip Sentry.
            log.warning("GraphQL bad query", exc_info=original_error)
        else:
