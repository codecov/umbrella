...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        if isinstance(original_error, BaseException) or isinstance(
            ...
            # GraphQL validation errors (e.g. "Cannot query field") are client
            # mistakes, not server errors — log but don't send to Sentry.
            log.warning("GraphQL bad query from anonymous user", exc_info=original_error)
        else:
