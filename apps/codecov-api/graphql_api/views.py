...
class AsyncGraphqlView(GraphQLAsyncView):
    ...
    def error_formatter(self, error: Any, debug: bool = False) -> dict[str, Any]:
        ...
        # GraphQL validation errors (e.g. "Cannot query field", "Variable '$x' is
        # never used") have no original_error — they are raised by the graphql-core
        # validation layer before any resolver runs, meaning they are always caused
        # by a malformed client query, not a server bug.
        original_error = error.original_error
        is_validation_error = original_error is None
        if debug or (not is_anonymous and is_validation_error):
            return format_error(error, debug)
        if is_validation_error:
            # Return the validation message as-is to the client (even anonymous)
            # but do NOT log or send to Sentry — this is a client error.
