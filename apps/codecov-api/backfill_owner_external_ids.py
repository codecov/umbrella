...
def backfill_owner_external_ids(batch_size: int = BATCH_SIZE) -> int:
    ...
    processed = 0
    progress = (
        tqdm(total=total, unit="owner", smoothing=0.1, dynamic_ncols=True)
        if tqdm is not None
        else None
    )
    ...
    try:
        while True:
            ...
            # filter and `ownerid__gt` guarantees forward progress even as rows
            # are backfilled underneath us.
            batch = list(
            ...
            if progress is not None:
                ...
                logger.info(f"  {processed}/{total} ({pct:.1f}%)")
    finally:
        if progress is not None:
            progress.close()
