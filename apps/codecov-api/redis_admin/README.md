...
### Local dev seeding (shell_plus)

The frequency chart's repo column renders `service:owner/name` only
when the envelope's `repoid` resolves to an actual `Repository` row
in Postgres; without matching DB rows it falls back to the bare
numeric repoid, which is useless for spotting which repo a stuck
queue belongs to. The snippet below creates the rows AND pushes
envelopes referencing them, so what you see in the admin matches
what you'd see in production.

It's intentionally not committed as a Python module — it's just a
paste-and-go for `shell_plus`. Open a shell in the api container and
paste:
...
from redis_admin import conn

QUEUE = "bundle_analysis"


def ensure_repo(service, username, name):
    try:
        owner = Owner.objects.get(service=service, username=username)
    except Owner.DoesNotExist:
        owner = OwnerFactory(service=service, username=username)
    try:
        return Repository.objects.get(author=owner, name=name)
    except Repository.DoesNotExist:
        return RepositoryFactory(author=owner, name=name)


def envelope(task, **kwargs):
    body = base64.b64encode(json.dumps([[], kwargs, {}]).encode()).decode("ascii")
    return json.dumps(
        {"body": body, "headers": {"task": task, "id": str(uuid.uuid4())}}
    )


example = ensure_repo("github", "codecov", "example")
another = ensure_repo("github", "codecov", "another-repo")
gitlab = ensure_repo("gitlab", "other-org", "some-project")

bundle = "app.tasks.bundle_analysis.BundleAnalysisProcessor"
notify = "app.tasks.notify.NotifyTask"
commit_a = "0832c110a744ddb8185bfdf0524aad41d3c3d21a"
commit_b = "abcdef0123456789abcdef0123456789abcdef01"
commit_c = "deadbeef"

mix = [
    (5, bundle, {"repoid": example.repoid, "commitid": commit_a}),
    (3, bundle, {"repoid": example.repoid, "commitid": commit_b}),
    (2, notify, {"repoid": another.repoid, "commitid": commit_a}),
    (1, notify, {"repoid": gitlab.repoid, "commitid": commit_c, "pullid": 42}),
]

broker = conn.get_connection(kind="broker")
for n, task, kwargs in mix:
    for _ in range(n):
        broker.rpush(QUEUE, envelope(task, **kwargs))

print(f"seeded onto {QUEUE!r}; LLEN = {broker.llen(QUEUE)}")
print(f"  github:codecov/example        -> repoid={example.repoid}")
print(f"  github:codecov/another-repo   -> repoid={another.repoid}")
print(f"  gitlab:other-org/some-project -> repoid={gitlab.repoid}")
print(
    f"open: http://localhost:8000/admin/redis_admin/celerybrokerqueue/"
    f"?queue_name__exact={QUEUE}"
)
```

What that creates (idempotent — reruns reuse the same `Owner` /
`Repository` rows by `(service, username)` and `(author, name)`, so
the `repoid` values stay stable):
...
To reset the queue between runs (DB rows are kept so the chart's
repo column stays stable):
...
the api container. The drill-down URL printed at the end of the
snippet (`?queue_name__exact=<queue>`) bypasses the summary list
