...
class OwnerAdminTest(TestCase):
    ...
    def test_get_queryset_annotates_repository_count(self):
        owner_with_repos = OwnerFactory()
        RepositoryFactory(author=owner_with_repos)
        RepositoryFactory(author=owner_with_repos)
        owner_without_repos = OwnerFactory()

        request = RequestFactory().get(reverse("admin:codecov_auth_owner_changelist"))
        request.user = self.staff_user
        annotated = {
            owner.ownerid: owner.repository_count
            for owner in self.owner_admin.get_queryset(request)
        }

        assert annotated[owner_with_repos.ownerid] == 2
        # Coalesced to 0 (not None) so the column renders and ordering works.
        assert annotated[owner_without_repos.ownerid] == 0
