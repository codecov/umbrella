...
class OwnerManagerTests(TestCase):
    ...
    def test_orgs_filters_exclude_to_be_deleted_owners(self):
        ...
        filtered_orgs_with_term = filtered_orgs.filter(username__contains="normal")
        ...
        # Test with term that matches no organizations
        filtered_orgs_no_match = filtered_orgs.filter(username__contains="nonexistent")
        no_match_ids = list(filtered_orgs_no_match.values_list("ownerid", flat=True))
        self.assertEqual(len(no_match_ids), 0)
