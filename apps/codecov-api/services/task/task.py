...
class TaskService:
    ...
    def _dispatch(self, sig, **apply_async_kwargs):
        """
        Dispatch a Celery signature (or group/chain) using a managed broker
        connection. Calling apply_async() directly from a web-server context
        (i.e. outside a Celery worker) can leave the Kombu Redis Channel with
        a None connection reference, causing an AttributeError. Using
        connection_or_connect() ensures a live connection is always provided.
        """
        with celery_app.connection_or_connect() as conn:
            return sig.apply_async(connection=conn, **apply_async_kwargs)
    ...
    def compute_comparisons(self, comparison_ids: list[int]):
        ...
        if len(comparison_ids) > 0:
            ...
            self._dispatch(group(signatures))
    ...
    def refresh(
        ...
        return self._dispatch(chain(*chain_to_call))
