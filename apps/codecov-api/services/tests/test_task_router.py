...
def test_get_owner_from_ownerid(fake_owners):
    (owner, owner_enterprise_cloud) = fake_owners
    assert (
        _get_owner_from_ownerid(owner.ownerid) == owner.ownerid
    )
    assert (
...
def test_get_owner_from_repoid(fake_repos):
    ...
    assert _get_owner_from_repoid(repo_enterprise.repoid) == repo_enterprise.author.ownerid
    assert _get_owner_from_repoid(10000000) == None

def test_get_user_plan_from_comparison_id(fake_compare_commit):
    (compare_commit, compare_commit_enterprise, repo, repo_enterprise) = fake_compare_commit
    assert (
...
def test_get_owner_from_comparison_id(fake_compare_commit):
    ...
    assert _get_owner_from_comparison_id(compare_commit_enterprise.id) == repo_enterprise.author.ownerid
    assert _get_owner_from_comparison_id(10000000) == None
...
def test_get_owner_from_task(
    ...
    assert _get_owner_from_task("unknown task", task_kwargs) == None
...
def test_route_task(mocker, fake_repos):
    ...
    mock_route_tasks_shared.assert_called_with(
        shared_celery_config.upload_task_name, PlanName.CODECOV_PRO_MONTHLY.value, repo.author.ownerid
    )
