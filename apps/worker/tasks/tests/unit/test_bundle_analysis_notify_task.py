...
def test_bundle_analysis_notify_lock_retry_limit_exceeded_during_token_acquisition(
    ...
    """Test that LockRetryLimitExceededError during token acquisition returns graceful failure."""
    mocker.patch.object(BundleAnalysisNotifyTask, "app", celery_app)
    ...
    # Mock debouncer's notification_lock to raise LockRetryLimitExceededError
    mock_notification_lock = mocker.patch.object(
        task.debouncer,
        "notification_lock",
        side_effect=LockRetryLimitExceededError("Max retries exceeded"),
    )
...
def test_bundle_analysis_notify_lock_retry_limit_exceeded_during_notification(
    ...
    """Test that LockRetryLimitExceededError during notification lock returns graceful failure."""
    mocker.patch.object(BundleAnalysisNotifyTask, "app", celery_app)
    ...
    # Mock debouncer's notification_lock to raise LockRetryLimitExceededError during notification
    mocker.patch.object(
        task.debouncer,
        "notification_lock",
        side_effect=LockRetryLimitExceededError("Max retries exceeded"),
