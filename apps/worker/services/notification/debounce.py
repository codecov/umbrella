...
class LockRetryLimitExceededError(Exception):
    """Raised when lock acquisition retries exceed the maximum allowed."""
    ...
    def notification_lock(self, lock_manager: LockManager, task_attempts: int):
        ...
        try:
            ...
            if e.max_retries_exceeded:
                ...
                raise LockRetryLimitExceededError(
                    f"Lock acquisition retry limit exceeded: {e.retry_num} >= {e.max_attempts}",
                )
            raise
