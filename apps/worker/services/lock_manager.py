...
from redis.exceptions import ResponseError as RedisResponseError  # type: ignore
...
class LockManager:
    ...
    def locked(
        ...
        try:
            ...
            try:
                lock_ctx = self.redis_connection.lock(
                    lock_name,
                ...
                log.warning(
                    "Failed to acquire lock due to Redis read-only replica",
                    extra={
                        "commitid": self.commitid,
                        "lock_name": lock_name,
                        "lock_type": lock_type.value,
                ...
                raise LockError("Redis read-only replica; cannot acquire lock")
            ...
            try:
                ...
                try:
                    ...
                    log.warning(
                        "Failed to release lock due to Redis read-only replica",
                        extra={
                            "commitid": self.commitid,
                            "lock_name": lock_name,
                            "lock_type": lock_type.value,
