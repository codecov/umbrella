...
def sample_report_for_components():
    ...
    first_file.append(2, ReportLine.create(1, sessions=[[0, 1]]))
    second_file = ReportFile("folder/poker2.py")
    second_file.append(3, ReportLine.create(0, sessions=[[0, 0]]))
    second_file.append(4, ReportLine.create(1, sessions=[[0, 1]]))
    third_file = ReportFile("random.go")
    third_file.append(5, ReportLine.create(0, sessions=[[0, 0]]))
    third_file.append(6, ReportLine.create(0, sessions=[[0, 0]]))
    third_file.append(8, ReportLine.create(0, sessions=[[0, 1]]))
    third_file.append(7, ReportLine.create(1, sessions=[[0, 0]]))
    report.append(first_file)
    report.append(second_file)
...
class TestTimeseriesService:
    ...
    def test_commit_measurement_update_component_parallel(
        self,
        sample_report_for_components,
        repository,
        dataset_names,
        ...
        measurements = (
            dbsession.query(Measurement)
            .filter_by(name=MeasurementName.component_coverage.value)
            .all()
        )

        assert len(measurements) == 1
        dbsession.add(commit)
        assert measurements[0].name == MeasurementName.component_coverage.value
        assert measurements[0].owner_id == commit.repository.ownerid
        assert measurements[0].repo_id == commit.repoid
