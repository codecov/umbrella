...
def get_testruns_for_uploads(
    ...
    """Fetch all testruns for the given uploads in a single query, grouped by upload_id."""
...
def handle_pass(
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        expiring_flakes.append(curr_flakes[test_id])
    ...
    for testrun in testruns:
        ...
        match testrun.outcome:
            case "pass":
                ...
                handle_pass(curr_flakes, test_id, expiring_flakes)
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Fetch all testruns for all uploads in a single batched query
    ...
    for upload in uploads:
        ...
        process_single_upload(upload_testruns, curr_flakes, repo_id, expiring_flakes)
    ...
    # Bulk-update all modified testrun outcomes in one query
