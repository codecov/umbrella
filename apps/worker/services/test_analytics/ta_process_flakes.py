...
def _process_testruns(
    testruns: QuerySet[Testrun],
    curr_flakes: dict[bytes, Flake],
    repo_id: int,
) -> list[Testrun]:
    """Apply flake outcome logic to all testruns in memory; returns the list for bulk_update."""
    testrun_list = list(testruns)
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    testrun_list = _process_testruns(testruns, curr_flakes, repo_id)

    # Update all testruns in a single bulk_update
    Testrun.objects.bulk_update(testrun_list, ["outcome"])
