...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        ...
        process_single_upload(upload_testruns, curr_flakes, expired_flakes, repo_id)
        log.info(
            "process_flakes_for_commit: processed upload",
            extra={"upload": upload.id},
        )

    # Bulk update testrun outcomes changed during processing
    Testrun.objects.bulk_update(all_testruns, ["outcome"])

    # Bulk save flakes that have been expired (reached 30 recent passes)
    if expired_flakes:
