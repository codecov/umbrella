...
def get_testruns_for_uploads(
    ...
    returning them grouped by upload_id."""
...
def process_single_upload(
    ...
    repo_id: int,
    expired_flakes: list[Flake],
    all_modified_testruns: list[Testrun],
):
    for testrun in testruns:
    ...
    all_modified_testruns.extend(testruns)
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        ...
        process_single_upload(
            testruns, curr_flakes, repo_id, expired_flakes, all_modified_testruns
        )
        log.info(
    ...
    # Bulk-update testruns whose outcome changed to "flaky_fail" in one query
    if all_modified_testruns:
        Testrun.objects.bulk_update(all_modified_testruns, ["outcome"])

    # Bulk-update flakes that have expired (reached 30 recent passes) in one query
    if expired_flakes:
