...
def get_testruns_for_uploads(
    ...
    """Fetch all testruns for the given uploads in a single query and
    return them grouped by upload_id."""
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Bulk-update flakes that expired (reached 30 consecutive passes) instead of
    # saving them one-by-one inside handle_pass.
    if expired_flakes:
