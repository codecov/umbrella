...
def get_testruns(upload_ids: list[int]) -> dict[int, list[Testrun]]:
...
def handle_pass(curr_flakes: dict[bytes, Flake], test_id: bytes):
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        # do not call .save() here; the bulk_create(update_conflicts=True) at the
        # end of process_flakes_for_commit will persist the end_date update.
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Single bulk query for all testruns across all uploads
    testruns_by_upload = get_testruns(upload_ids)
