...
def get_testruns_for_uploads(
    ...
    """Fetch all testruns for a set of uploads in a single query and group by upload_id."""
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Bulk-fetch all testruns for all uploads in a single query
    ...
    # Bulk-update expired flakes instead of saving individually
    if expired_flakes:
