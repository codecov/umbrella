...
def process_all_uploads(
    uploads: QuerySet[ReportSession], curr_flakes: dict[bytes, Flake], repo_id: int
):
    upload_ids = list(uploads.values_list("id", flat=True))
    testruns = list(get_all_testruns(upload_ids))
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    process_all_uploads(uploads, curr_flakes, repo_id)
    log.info(
