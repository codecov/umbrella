...
def get_testruns_for_uploads(upload_ids: list[int]) -> list[Testrun]:
...
def handle_pass(
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        # Defer the save; caller will bulk_update all expired flakes at once
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Fetch all testruns for all uploads in a single query, then group in Python
    ...
    # Bulk update all expired flakes (those that reached 30 passes) in one query
    if expired_flakes:
        Flake.objects.bulk_update(
            expired_flakes, ["end_date", "count", "recent_passes_count"]
        )

    # Bulk update all modified testruns across all uploads in one query
