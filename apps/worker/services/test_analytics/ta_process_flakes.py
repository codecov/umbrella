...
def get_testruns_for_uploads(
    uploads: Sequence[ReportSession],
) -> dict[int, list[Testrun]]:
    """Fetch all testruns for a collection of uploads in a single query and group by upload_id."""
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Bulk-save flakes that expired (hit 30 recent passes) instead of per-row saves
    if expired_flakes:
        Flake.objects.bulk_update(expired_flakes, ["end_date", "count"])
