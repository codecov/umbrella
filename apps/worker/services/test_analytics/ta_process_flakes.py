...
def get_testruns(uploads: QuerySet[ReportSession]) -> QuerySet[Testrun]:
    # Materialize IDs to a plain list to avoid a cross-database subquery
    # (ReportSession lives in `default`, Testrun lives in `ta_timeseries`)
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    log.info(
        ...
        extra={"uploads": list(uploads.values_list("id", flat=True))},
    )
    ...
    testruns = list(get_testruns(uploads))
