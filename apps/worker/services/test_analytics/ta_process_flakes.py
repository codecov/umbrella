...
def get_testruns_for_uploads(
    upload_ids: Sequence[int],
) -> dict[int, list[Testrun]]:
    """Fetch all testruns for a batch of upload IDs in a single query,
    returning a dict mapping upload_id -> list of Testrun."""
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        process_single_upload(upload, testruns_by_upload[upload.id], curr_flakes, repo_id)
        log.info(
