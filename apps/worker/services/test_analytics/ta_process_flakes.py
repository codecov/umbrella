...
def process_flakes_for_commit(
    ...
    for upload in uploads:
        process_single_upload(
            testruns_by_upload.get(upload.id, []),
            curr_flakes,
            expired_flakes,
            repo_id,
        )
    ...
    try:
        with redis_client.lock(lock_name, timeout=300, blocking_timeout=3):
            while commit_ids := redis_client.lpop(key_name, 10):
                # Fetch flakes once per batch of commits rather than once per commit
                ...
                log.info(
                    ...
                    extra={
                        "flakes": [
                            flake.test_id.hex() for flake in curr_flakes.values()
                        ]
                    },
                )
                ...
                log.info(
                    "process_flakes_for_repo: bulk saving flakes",
                    extra={
                        "active_flakes": [
                            flake.test_id.hex() for flake in curr_flakes.values()
                        ],
                        "expired_flakes": [
                            flake.test_id.hex() for flake in expired_flakes
                        ],
                    },
                )

                # Bulk-save all active flakes (upsert) in one query
                ...
                # Bulk-update expired flakes (end_date set) in one query
                if expired_flakes:
