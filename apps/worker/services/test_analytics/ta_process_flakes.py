...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        process_single_upload(
            upload, testruns_by_upload[upload.id], curr_flakes, repo_id, expired_flakes
        )
        log.info(
    ...
    # Bulk-update flakes that expired (hit 30 recent passes) instead of saving one by one
    if expired_flakes:
