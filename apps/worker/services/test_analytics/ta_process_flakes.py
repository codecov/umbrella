...
def handle_pass(
    ...
    ended, so the caller can bulk-update it."""
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        ...
        process_single_upload(testruns, curr_flakes, repo_id, expired_flakes)
        log.info(
            "process_flakes_for_commit: processed upload",
            extra={"upload": upload.id},
        )

    # Bulk-update all flakes that expired (reached 30 recent passes) in one query
    if expired_flakes:
