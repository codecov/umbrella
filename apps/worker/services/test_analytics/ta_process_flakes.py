...
def handle_pass(
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        expiring_flakes.append(curr_flakes.pop(test_id))
    ...
    for testrun in testruns:
        ...
        match testrun.outcome:
            case "pass":
                if test_id not in curr_flakes:
                    continue
                handle_pass(curr_flakes, expiring_flakes, test_id)
            case "failure" | "flaky_fail" | "error":
                handle_failure(curr_flakes, test_id, testrun, repo_id)
            case _:
                continue

    # Bulk update all testrun outcomes in one query
    Testrun.objects.bulk_update(testruns, ["outcome"])

    # Bulk save expired flakes in one query
    if expiring_flakes:
