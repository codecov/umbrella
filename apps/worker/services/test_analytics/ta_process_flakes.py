...
def get_testruns_for_uploads(
    upload_ids: list[int],
) -> defaultdict[int, list[Testrun]]:
    """Fetch all testruns for the given upload IDs in a single query,
    grouped by upload_id to avoid N+1 queries."""
    testruns_by_upload: defaultdict[int, list[Testrun]] = defaultdict(list)
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        ...
        process_single_upload(testruns, curr_flakes, repo_id)
