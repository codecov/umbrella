...
def get_testruns(upload: ReportSession) -> list[Testrun]:
    ...
    for testrun in testruns:
        ...
        match testrun.outcome:
            ...
            case "failure" | "flaky_fail" | "error":
                if handle_failure(curr_flakes, test_id, testrun, repo_id):
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        testruns = process_single_upload(upload, curr_flakes, repo_id, expired_flakes)
        testruns_to_update.extend(testruns)
        log.info(
