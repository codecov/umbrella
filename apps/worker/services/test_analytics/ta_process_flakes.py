...
def get_testruns_for_uploads(
    ...
    """Fetch all testruns for a list of upload IDs in a single query,
    returning a dict keyed by upload_id."""
    testruns: dict[int, list[Testrun]] = {uid: [] for uid in upload_ids}
    ...
    for testrun in qs:
        testruns[testrun.upload_id].append(testrun)
    return testruns
...
def handle_pass(
    ...
    """Handle a passing testrun. Returns the expired Flake if it should be
    ended (so the caller can bulk-update), otherwise None."""
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        expired = curr_flakes.pop(test_id)
        return expired
    return None
    ...
    for testrun in testruns:
        ...
        match testrun.outcome:
            case "pass":
                ...
                expired = handle_pass(curr_flakes, test_id)
                if expired is not None:
                    expired_flakes.append(expired)
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        process_single_upload(
            testruns_by_upload[upload.id], curr_flakes, repo_id, expired_flakes
        )
        log.info(
    ...
    # Bulk-update flakes that were expired (reached 30 passes) during processing
    if expired_flakes:
