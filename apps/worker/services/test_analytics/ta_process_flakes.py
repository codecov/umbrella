...
FAIL_FILTER = Q(outcome__in=["failure", "flaky_fail", "error"])


def get_testruns(
    upload_ids: list[int], curr_flake_test_ids: list[bytes]
) -> QuerySet[Testrun]:
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Group testruns by upload_id so we can bulk_update per upload.
