...
def get_testruns_for_uploads(uploads: list[ReportSession]) -> list[Testrun]:
    """Fetch all testruns for a list of uploads in a single query."""
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Batch-fetch all testruns for all uploads in a single query, then group by upload_id
    ...
    # Bulk-update expired flakes (those that reached 30 passes) in one query
    if expired_flakes:
