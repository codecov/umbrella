...
def process_flakes_for_commit(
    ...
    if not uploads:
        log.info("process_flakes_for_commit: no uploads found, skipping")
        return
    ...
    log.info(
        ...
        extra={"testrun_count": sum(len(runs) for runs in testruns_by_upload.values())},
    )
...
def process_flakes_for_repo(repo_id: int):
    ...
    try:
        with redis_client.lock(lock_name, timeout=300, blocking_timeout=3):
            ...
            log.info(
                "process_flakes_for_repo: bulk creating/updating flakes",
                extra={
                    "repo_id": repo_id,
                    "flake_count": len(curr_flakes),
                },
