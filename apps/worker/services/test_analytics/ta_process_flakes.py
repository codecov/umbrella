...
def get_relevant_uploads(repo_id: int, commit_id: str):
...
def process_single_upload(
    ...
    """Process flake logic for a pre-fetched list of testruns. Returns testruns with updated outcomes."""
    updated_testruns: list[Testrun] = []
        ...
        match testrun.outcome:
            ...
            case "failure" | "flaky_fail" | "error":
                ...
                if testrun.outcome != original_outcome:
                    updated_testruns.append(testrun)
            case _:
                continue

    return updated_testruns
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    all_updated_testruns: list[Testrun] = []
    expired_flakes: list[Flake] = []

    for upload in uploads:
        upload_testruns = testruns_by_upload.get(upload.id, [])
        updated = process_single_upload(
            upload_testruns, curr_flakes, repo_id, expired_flakes
        )
        all_updated_testruns.extend(updated)
        log.info(
            "process_flakes_for_commit: processed upload",
            extra={"upload": upload.id},
        )

    if all_updated_testruns:
        Testrun.objects.bulk_update(all_updated_testruns, ["outcome"])
