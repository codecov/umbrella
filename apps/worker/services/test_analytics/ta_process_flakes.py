...
def get_testruns_for_uploads(
    ...
    """Bulk-fetch testruns for all uploads in a single query and group by upload_id."""
...
def handle_pass(
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        # Move to expired_flakes so bulk_create can still persist the end_date
        # without issuing an individual save() per flake.
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    all_flakes = list(curr_flakes.values()) + expired_flakes
    log.info(
        "process_flakes_for_commit: bulk creating flakes",
        extra={"flakes": [flake.test_id.hex() for flake in all_flakes]},
    )
