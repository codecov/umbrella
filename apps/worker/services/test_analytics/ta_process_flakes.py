...
def handle_pass(
    curr_flakes: dict[bytes, Flake], test_id: bytes, expired_flakes: dict[bytes, Flake]
):
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        ...
        process_single_upload(
            upload, curr_flakes, repo_id, expired_flakes, upload_testruns
        )
        log.info(
    ...
    if expired_flakes:
        log.info(
            ...
            extra={
                "expired_flakes": [
                    flake.test_id.hex() for flake in expired_flakes.values()
                ]
            },
        )
        Flake.objects.bulk_update(
            expired_flakes.values(),
            ["end_date", "count", "recent_passes_count", "fail_count"],
        )
