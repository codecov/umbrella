...
def get_all_testruns(uploads: QuerySet[ReportSession]) -> list[Testrun]:
...
def process_single_testrun(
    testrun: Testrun, curr_flakes: dict[bytes, Flake], repo_id: int
):
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    all_testruns = get_all_testruns(uploads)

    for testrun in all_testruns:
        process_single_testrun(testrun, curr_flakes, repo_id)

    log.info(
        "process_flakes_for_commit: processed all uploads",
