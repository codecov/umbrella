...
def handle_pass(
    ...
    expired_flakes: dict[bytes, Flake],
    test_id: bytes,
):
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        # defer the DB write — collect for bulk_update later
        expired_flakes[test_id] = curr_flakes[test_id]
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Fetch all testruns for every upload in a single query, then group in memory
    upload_ids = [upload.id for upload in uploads]
    all_testruns = list(get_testruns_for_uploads(upload_ids))
    testruns_by_upload: dict[int, list[Testrun]] = {upload.id: [] for upload in uploads}
    for testrun in all_testruns:
        testruns_by_upload[testrun.upload_id].append(testrun)

    for upload in uploads:
        process_single_upload(
            upload, testruns_by_upload[upload.id], curr_flakes, expired_flakes, repo_id
        )
        log.info(
    ...
    # Bulk-update flakes that expired (reached 30 recent passes) instead of
    # individual save() calls that were previously fired inside the testrun loop.
    if expired_flakes:
        Flake.objects.bulk_update(
            expired_flakes.values(),
