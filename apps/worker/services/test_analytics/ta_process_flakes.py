...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Group testruns by upload_id so we can process them per-upload without extra queries
    ...
    # Bulk-update all testruns across all uploads in a single query
    ...
    Flake.objects.bulk_create(
        [*curr_flakes.values(), *expired_flakes],
