...
def get_relevant_uploads(repo_id: int, commit_id: str) -> QuerySet[ReportSession]:
    recent_testruns = Testrun.objects.filter(
    ...
    return ReportSession.objects.filter(
        ...
        Prefetch("testrun_set", queryset=recent_testruns, to_attr="recent_testruns")
    )
...
def get_testruns(upload: ReportSession) -> list[Testrun]:
    if hasattr(upload, "recent_testruns"):
        return upload.recent_testruns
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    log.info(
        "process_flakes_for_commit: bulk creating/updating flakes",
        extra={"flakes": [flake.test_id.hex() for flake in curr_flakes.values()]},
    )
