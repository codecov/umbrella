...
def get_testruns_for_uploads(
    ...
    """Bulk-fetch all testruns for a collection of uploads in a single query,
    returning a mapping of upload_id -> list[Testrun]."""
...
def handle_pass(curr_flakes: dict[bytes, Flake], test_id: bytes):
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        # do not save individually here; the final bulk_create with
        # update_conflicts=True in process_flakes_for_commit will persist end_date.
