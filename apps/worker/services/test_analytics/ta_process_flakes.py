...
def handle_pass(curr_flakes: dict[bytes, Flake], test_id: bytes):
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        # Mark as expired; bulk_create at the end of process_flakes_for_commit
        # will persist end_date without requiring an individual save() call.
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        process_single_upload(
            upload, testruns_by_upload.get(upload.id, []), curr_flakes, repo_id
        )
        log.info(
