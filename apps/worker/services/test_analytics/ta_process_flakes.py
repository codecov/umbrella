...
def get_testruns_for_uploads(
    ...
    """Batch-fetch all testruns for a list of uploads in a single query,
...
def handle_pass(curr_flakes: dict[bytes, Flake], test_id: bytes):
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        # Mark the flake as expired; the bulk_create/update at the end of
        # process_flakes_for_commit will persist end_date without an extra save().
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    # Batch-fetch all testruns for all uploads in a single query to avoid N+1.
    testruns_by_upload = get_testruns_for_uploads(uploads)

    all_testruns: list[Testrun] = []
    for upload in uploads:
        upload_testruns = process_single_upload(
            upload, curr_flakes, repo_id, testruns=testruns_by_upload.get(upload.id, [])
        )
    ...
    # Persist outcome changes for all testruns in one bulk_update.
