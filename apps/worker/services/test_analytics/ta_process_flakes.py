...
def get_testruns_for_uploads(
    ...
    """Fetch all testruns for a batch of uploads in a single query, grouped by upload_id."""
...
def handle_pass(curr_flakes: dict[bytes, Flake], test_id: bytes):
    ...
    if curr_flakes[test_id].recent_passes_count == 30:
        ...
        # flake will be persisted by the bulk_create at the end of process_flakes_for_commit
...
def process_flakes_for_commit(repo_id: int, commit_id: str):
    ...
    for upload in uploads:
        modified = process_single_upload(
