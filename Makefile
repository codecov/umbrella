...
DOCKERHUB_REPO ?= codecov/self-hosted-umbrella
...
test_env.prepare:
	docker-compose exec umbrella make test_env.container_prepare
...
test_env.container_upload:
	...
	--coverage-files-search-exclude-folder=graphql_umbrella/types/** \
	--coverage-files-search-exclude-folder=umbrella/internal/tests/unit/views/cassetes/**

test_env.container_upload_test_results:
	codecovcli -u ${CODECOV_URL} do-upload --report-type "test_results" \
	--files-search-exclude-folder=graphql_umbrella/types/** \
	--files-search-exclude-folder=umbrella/internal/tests/unit/views/cassetes/** || true
...
test_env.ats:
	docker-compose exec umbrella make test_env.container_ats CODECOV_UPLOAD_TOKEN=${CODECOV_UPLOAD_TOKEN}
